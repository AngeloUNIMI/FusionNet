function [ROIr, errorC, resultsROI, rotPalm, rotBw, top1, top2, bottom1, bottom2, grad, gradRefined] = findROI_fromShape(im, bw, shapeF, centroid, param, dbname, filename, jpgFiles, savefile, plotta)

%partially based on:
%D. Zhang, Wai-Kin Kong, J. You and M. Wong,
%"Online palmprint identification,"
%in IEEE Transactions on Pattern Analysis and Machine Intelligence,
%vol. 25, no. 9, pp. 1041-1050, Sept. 2003.
...
for iterations = 1 : param.peakFind.numIterCentroid
    ...
    %Higher order Moving average filter used... User may wish to change the parameters of the above function
    % [~, i, ~, p] = findpeaks(smooth(-regionLine(:,3), smoothF), 'MinPeakDistance', 2, 'MinPeakHeight', mean(-regionLine(:,3)));
    % [~, i, ~, p] = findpeaks(smooth(-regionLine(:,3), smoothF), 'MinPeakDistance', 0);
    % [~, i, ~, p] = findpeaks(smooth(-regionLine(:,3), smoothF), 'MinPeakDistance', 50, 'MinPeakHeight', mean(-regionLine(:,3)));
    % [~, i, ~, p] = findpeaks(smooth(-regionLine(:,3), smoothF), 'MinPeakDistance', 50);
    ...
    %     %remove last peak in order of height
    %     [~, ind_p_sorted] = sort(pks, 'descend');
    %     if numel(ind_peaks) >= 5
    %         ind_peaks = ind_peaks(ind_p_sorted(1:end-1));
    %     end %if numel(ind_peaks) > 3
    ...
    %     figure,
    %     findpeaks(smoothedDistPeaks, 'MinPeakDistance', param.peakFind.minPeakDistance, 'MinPeakProminence', 5, 'MinPeakHeight', meanP, 'Annotate', 'extents')
    %     pause
    ...
    % assignin('base', 'regionLine', regionLine);
    % assignin('base', 'smoothF', smoothF);
    % assignin('base', 'coordinates', coordinates);
    % assignin('base', 'sortCoord', sortCoord);
    % pause
    ...
    %     imshow(im)
    %     hold on
    %     plot(sortCoord(i_p1, 1), sortCoord(i_p1, 2), 'rx');
    %     plot(sortCoord(i_p2, 1), sortCoord(i_p2, 2), 'rx');
    ...
    [ROIout, rotPalm, rotBw, new_c, top1, top2, bottom1, bottom2, errorC(2)] = ...
